...
try {
    ...
    global.__expo &&
        ...
        MaterialCommunityIcons = ({ name, color, size, ...rest }) => {
            ...
            return (React.createElement(Text, Object.assign({}, rest, { style: [styles.icon, { color, fontSize: size }], 
                // @ts-ignore
                pointerEvents: "none" }), "\u25A1"));
        };
    }
...
const defaultIcon = ({ name, color, size, direction, allowFontScaling, }) => (React.createElement(MaterialCommunityIcons, Object.assign({ allowFontScaling: allowFontScaling, name: name, color: color, size: size, style: [
        {
