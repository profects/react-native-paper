...
export default function ExampleList({ navigation }) {
    const renderItem = ({ item }) => (React.createElement(List.Item, { title: item.data.title, onPress: () => navigation.navigate(item.id) }));
    const keyExtractor = (item) => item.id;
    const { colors } = useTheme();
    const safeArea = useSafeArea();
    return (React.createElement(FlatList, { contentContainerStyle: {
            backgroundColor: colors.background,
            paddingBottom: safeArea.bottom,
        }, ItemSeparatorComponent: Divider, renderItem: renderItem, keyExtractor: keyExtractor, data: data }));
}
